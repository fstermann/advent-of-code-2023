...
class Game:
    ...
    def min_cubes(self) -> CubeSet:
        return CubeSet(
            max(cube_set.red for cube_set in self.cubesets),
            max(cube_set.green for cube_set in self.cubesets),
            max(cube_set.blue for cube_set in self.cubesets),
        )
...
def run(input_: str) -> int:
    ...
    return sum(game.min_cubes().mult() for game in games)
